...
<?php
switch($_SESSION['nivel']) //Aqui faltan los datos de todos los niveles!!!!!!*******************
{
	case 1:
	{
...
$c_grado = 'espacios_3'; $ee = 'espacios_extra3';
$error = 0;
$msn = "";
...
if (isset($_POST['guardar']))
{
	...
	//->CREAR CURP
	$rear = "000000";
	$nom_a = stripAccents($nom);
	$nom_a = substr($nom_a,0,2);
	
	$app_a = stripAccents($app);
	$app_a = substr($app_a,0,2);
	if ($apm == "")
	{
		$apm_a = "XX";
	}
	else
	{
		...
		$apm_a = stripAccents($apm);
		$apm_a = substr($apm_a,0,2);
	}
	$i = 1;
	while ($i)
	{
		...
		$curp_a = "000000".$nom_a.$app_a.$apm_a.$rear;
		$sql = "SELECT * FROM t_alumnos WHERE id_alum = '".$curp_a."'";
		require ("consulta.php");
		if (!$zero_results)
		{
			$rear = $rear + 1;
	...
	echo '<script>alert ("La CURP PROVISIONAL con la que se realizará el cambios es '.$curp_a.'. Esta CURP NO tiene validez oficial.");</script>';
	$_SESSION['curp'] = $curp_a;
	//->
	$sql = "SELECT * FROM t_alumnos WHERE nom = '".$nom."' AND app = '".$app."' AND apm = '".$apm."'";
	require ('consulta.php');
	if (!$zero_results) //if already exist
	{
		...
		{
			echo '<script>alert ("Un alumno con el mismo nombre y apellidos ya está registrado con cambio en '.$nivel_o.' en la '.$sub_o.' en la escuela '.$cct_ou.'; verifique que no se trata del mismo alumno. En caso de que sea el mismo favor de borrar el cambios anterior.");</script>';
		}
		else
		{
	...
	else //if not
	{
		...
		if ($_SESSION['aux'] == 0)
		{
			...
			if ($cct_org == "NULL")
			{$sql = "INSERT INTO t_alumnos VALUES ('".$curp_a."','".$nom."','".$app."','".$apm."',NULL,'".$cct_ou."',DEFAULT,CURDATE(),'TERCERO','".$_SESSION['user']."',0,NULL,NULL,0,'".$temp."',".$temp_aux.")";}
			else
			{$sql = "INSERT INTO t_alumnos VALUES ('".$curp_a."','".$nom."','".$app."','".$apm."','".$cct_org."','".$cct_ou."',DEFAULT,CURDATE(),'TERCERO','".$_SESSION['user']."',0,NULL,NULL,0,'".$temp."',".$temp_aux.")";}
			require ('consulta.php');
...
<br><br>
<fieldset>
<legend><?php echo $_SESSION['user']; ?> - Inscripción <u>TERCER</u> Grado <u>SIN CURP</u></legend>
<form action="cambios3a.php" method="post" name="cambios3a" id="cambios3a">
<div align = "left">
<br><div align = "center">La CURP que se cree PROVISIONALMENTE <u>NO TIENE VALIDEZ OFICIAL</u> ni podrá usarse para ningún otro trámite.</div><br><br>
...
<?php if ($_SESSION['flag']==1) {?>
	...
	<td><input name="curp" type="text" id="curp" class="daton" title="Escriba el CURP" value="<?php if($_SESSION['curp'] != ""){echo $_SESSION['curp'];}?>" style="width:250px" maxlength="18" placeholder="CURP" onkeyup="this.value = this.value.toUpperCase()" disabled="disabled" onclick="_quita('p_curp')"/></td>
	</tr>
<?php } ?>
<tr>
...
<form method="post" action="cambios3a.php">
